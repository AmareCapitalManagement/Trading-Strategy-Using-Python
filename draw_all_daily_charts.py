...
def draw_all_daily_charts(
    ...
    """
    For every ticker in tickers_notes draw and save two daily OHLC + VWAPs charts.
    Chart 1: with year's 1st day VWAP added.
    Chart 2: without year's 1st day VWAP.

    Fill the tickers_follow_daily.xlsx file with the tickers we are interested in, their notes and anchor dates.
    We can add zero, one, or several custom anchor dates to each ticker.
    Please note that anchor dates for the last recent low and high are added automatically, so we don't have to add and update them manually. 
    """
    ...
    for ticker in tickers_notes["Ticker"].values:
        ...
        ohlc_df = get_ohlc-func(ticker=ticker, period="max", interval=interval)

        # adding our custom anchor dates for the ticker if they are available.
        custum_anchor_dates = list()
        ...
        ticker_note = ticker_notes.loc[ticker_notes["Ticker"] == ticker, "Note"].values[0]
        if ticker_note != "":
