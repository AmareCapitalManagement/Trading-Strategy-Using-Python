...
def create_candlestick_volume_chart(
    ...
    # Ensure index is a DatetimeIndex and timezone-naive
    ...
    # Try rendering the candlestick chart
    try:
        fig.add_trace(
            go.Candlestick(
                x=df.index,
        ...
        # Fallback: Plot closing prices as a line
        fig.add_trace(
            go.Scatter(
                x=df.index,
                y=df["Close"],
    ...
    # Determine if the data is intraday or daily
    if len(df) > 1:
    ...
    # Standalone candlestick chart for debugging
    ...
    standalone_fig.update_layout(
        xaxis=dict(range=[df.index.min(), df.index.max()], tickformat="%b %Y"),
        yaxis=dict(range=[min(df["Low"]), max(df["High"])]),
    )
    #standalone_fig.write_image(f"standalone_candlestick_{ticker}.png")
